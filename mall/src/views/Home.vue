<template>
  <div id="home"
    ...
    <div class="brandlist">
      <van-panel title="品牌制造商直供">
        <van-grid :column-num="2">
          <van-grid-item v-for="(item1,index1) in brandList"
                         :key="index1">
            <van-image fit="cover"
                       lazy-load
    ...
    <div class="newlist">
      <van-panel title="品牌制造商直供">
        <van-grid :column-num="2">
          <van-grid-item v-for="(item2,index2) in newGoodsList"
                         :key="index2">
            <van-image fit="cover"
                       lazy-load
                       :src="item2.list_pic_url" />
    ...
    <div class="hotlist">
      <van-panel title="人气推荐">
        <van-card v-for="(item3,index3) in hotGoodsList"
                  :key="index3"
    ...
    <van-swipe :autoplay="3000"
      ...
      <van-swipe-item v-for="(image, index) in topicList"
                      :key="index">
        <img class="swiperimg"
             v-lazy="image.item_pic_url" />
        <div class="topiclistTitle">
          <h5>{{image.title}}</h5>
          <p>￥{{image.price_info}}起</p>
        </div>
        <p class="topiclistcontent van-ellipsis">{{image.subtitle}}</p>
      </van-swipe-item>
    </van-swipe>
    <!-- 商品列表 -->
    <div class="newlist">
      <van-panel :title="good.name"
                 v-for="(good,i) in categoryList"
                 :key="i">
        <van-grid :column-num="2">
          <van-grid-item v-for="(item2,index2) in good.goodsList"
                         :key="index2">
            <van-image fit="cover"
                       lazy-load
                       :src="item2.list_pic_url" />
